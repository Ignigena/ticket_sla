...
if ($('.folderBack .dTreeNode').length) {
    ...
    $("#mainDiv div#dparentTree0 > div:nth-child(1)").after('<div class="dTreeNode p0 toptier"><a id="alltickets" href="'+ticketViewURL+'" target="content" class="node">All New Tickets</a></div>');
...
if ($('div.ticketCell table:nth-child(1) td.head2').text().trim() == "Ticket Summary") {
    ...
    if ($('.customerToolkitHeader td').length) {
        ...
        if ($('.customerToolkitHeader td:contains("Account")+td a').attr('href')) {
        accountID = $('.customerToolkitHeader td:contains("Account")+td a').attr('href').split('?amID=')[1];
            if (accountID) {
