...
var outOfScopeRegex = /Out of Scope\?:&nbsp;<\/td><td>Yes/;
...
if (document.location.href.split('?')[0]) {
  ...
  if (urlElements[urlElements.length - 1] == "ticketDetail.asp") {
    ...
    if (ticketIDNumberMatch && ticketIDNumberMatch[1] && parent.frames.name != "root") {
...
if (ticketQueuesRegex.test(document.body.innerText)) {
  ...
  // Add a link to the "New and Unassigned" queue at the top of the list.
  $("#mainDiv").find("> div").prepend('<div class="dTreeNode p0 toptier"><img src="../images/ftv2blank.gif" alt=""><img class="nodeIcon" id="iparentTree2" src="/ics/images/ticket/ticketQueueClosed.gif" alt=""><a id="sparentTree2" href="' + ticketViewURL + '" target="content" class="node">All New Tickets</a></div>');
...
function processTicketList() {
  chrome.storage.local.get('newWindowTickets', function (data) {
    if (data.newWindowTickets) {
      ticketTable.find("tbody tr td a").each(function () {
  ...
  if (ticketListSLARegex.test(document.body.innerText)) {
    ticketTable.before('<table class="escalated-advocacy" width="100%"></table>');
  ...
  var headers = [];
  ticketTable.find('thead td').each(function (index, item) {
    headers[index] = $(item).text().trim();
  });
  ticketTable.find('tr.gridRow').has('td').each(function () {
    var arrayItem = {};
  ...
  if (tickets.length > 0 && 'Expiry Timestamp' in tickets[0]) {
    ...
    $("#lockedHeader").find("tr").prepend('<td class="winColName slacolumn" nowrap style="width:90px">Time to SLA</td>');
    ticketTable.find("thead tr").prepend('<td class="winColName slacolumn" nowrap style="width:90px">Time to SLA</td>');
    ...
    for (var i = 0; i < tickets.length; i++) {
      ...
      if (tickets[i]['Onboarding Account'] == 'Yes') {
        ticketTable.find('tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (getColumnIndexByName('Account Name') - 1) + ')').append(' <b class="onboarding">Onboarding</b>');
      }

      if (tickets[i]['Remote Administration'] == 'Yes') {
        ticketTable.find('tbody tr:nth-child(' + (i + 1) + ') td:nth-child(' + (getColumnIndexByName('Summary') - 1) + ')').append(' <b class="ra">(RA)</b>');
      }
...
// Ticketmine Shortcut!
var toolbarTab = $('#toolbar').find('td.tabs').next();
if (toolbarTab.text().trim() == "New Ticket") {
  $('td#folderTabDivider', toolbarTab).after('<td class="winButton ticketmine" nowrap="" style="cursor:hand;" onmouseover="this.className=\'winButtonActive\';" onmouseout="this.className=\'winButton\';"><img src="http://ticketmine.network.acquia-sites.com/sites/default/files/manatee.png" border="0" style="height:16px; width:16px;" align="absmiddle">&nbsp;Ticketmine&nbsp;</td>');
...
if ($('div.ticketCell table:nth-child(1) td.head2').text().trim() == "Ticket Summary") {
  ...
  if (title.text() != "Edit Ticket" && title.text() != "New Ticket") {
    ...
    var bugTrackerCell = $('td:contains("Bug Tracker URL:")');
    bugTrackerCell.waitFor(function () {
      var jiraTicket = bugTrackerCell.next().text();
      if (jiraTicket.toLowerCase().indexOf("http") >= 0) {
        var jiraTicketNumber = String(jiraTicket.split('/').pop());
  ...
  if (!ticketStatusRegex.test(document.body.innerText)) {
    ...
    if (expiryTimestamp) {
      ...
      $.when(slaStatus).done(function (status) {
        if (!status['response']) {
          var leftTicketColumn = $('#ticketLeftCol');

          if (outOfScopeRegex.test(document.body.innerHTML)) {
            leftTicketColumn.prepend('<div id="slaBanner" class="grey"><strong>This ticket has been marked as Out of Scope</strong> A response is still required.</div>');
          } else {
            leftTicketColumn.prepend('<div id="slaBanner" class="' + sla['color'] + '"><strong>Response required</strong> <abbr class="timeago" title="' + sla['timestamp'] + '">' + sla['timestamp'] + '</abbr></div>');
          }
          leftTicketColumn.css('padding-top', '40px');

          var relativeTime = $('abbr.timeago');
          $.timeago.settings.allowFuture = true;
          relativeTime.timeago();

          // Respond to changes in the SLA banner to avoid missing SLA from a forgotten tab.
          relativeTime.on('timechanged', function (event, timestamp, timeDifference) {
            var minuteDifference = (timeDifference / 1000) / 60;
          ...
          $('#slaBanner').css('width', '-webkit-calc(' + leftTicketColumn[0].style.width + ' - 22px)');
        }
      });
    }
  }
...
function ticketListUITidy(slaSort) {
  ...
  ticketTable.find('td img').remove();
  ...
  var tableContainer = $(".lockedTableContainer");
  tableContainer.height((tableContainer.height() - 16) + 'px');

  var tableHeader = ticketTable.find('thead tr').html();
  tableHeader = tableHeader.replace(/<td /g, '<th ').replace(/<\/td>/g, '</th>');
  ticketTable.find('thead tr').html(tableHeader);

  ticketTable.find("thead th").not(":has(input[name=allCheck])").attr('data-sort', 'string');
  ticketTable.find("thead .slacolumn").attr('data-sort', 'int');
  ticketTable.stupidtable();

  if (slaSort) {
    ticketTable.find('thead th.slacolumn').trigger('click');
  }
}
...
function changeTicketStatus(rowNumber, newStatus, minutes) {
  var listRow = $('#listRow' + rowNumber);
  var currentStatus = listRow.attr('class');
  currentStatus = currentStatus.replace(/\s/g, "").replace(/gridRow/g, "");

  listRow.removeClass(currentStatus).addClass(newStatus);
...
function hideColumnByColumnName(columnName) {
  for (var column in arguments) {
    if (!arguments.hasOwnProperty(column)) continue;
